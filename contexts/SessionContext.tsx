...
interface AuthContextType {
  ...
  updateSession: (data: Partial<Omit<UserSession, 'accessToken'>>) => void
...
export function SessionProvider({ children }: PropsWithChildren) {
  ...
  const signIn = async (data: LoginData): Promise<{ success: boolean; message?: string }> => {
    try {
      const response = (await api.post<SuccessResponse<UserSession>>('/auth/login', data, {
        requireAuth: false,
  ...
  const updateSession = (data: Partial<Omit<UserSession, 'accessToken'>>) => {
    if (session) {
